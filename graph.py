...
def optimize_graph(logger=None, verbose=False, pooling_strategy=PoolingStrategy.REDUCE_MEAN, max_seq_len=40):
    ...
    try:
        ...
        # allow_soft_placement:Automatic selection of operating equipment
        config = tf.ConfigProto(allow_soft_placement=True)
        config_fp = args.config_name
        init_checkpoint = args.ckpt_name
        logger.info('model config: %s' % config_fp)

        # Loading the bert configuration file
        ...
        with jit_scope():
            ...
            # Get all the variables to be trained
            ...
            with tf.variable_scope("pooling"):
                # If there is only one layer, only the weight of the corresponding layer is taken.
                if len(args.layer_indexes) == 1:
                    encoder_layer = model.all_encoder_layers[args.layer_indexes[0]]
                else:
                    # Otherwise iterate through the layers that need to be fetched,
                    # take out the weight of all the layers and splice them together shape:768*number of layers.
                    all_layers = [model.all_encoder_layers[l] for l in args.layer_indexes]
                    encoder_layer = tf.concat(all_layers, -1)

                input_mask = tf.cast(input_mask, tf.float32)

                # The following code is a method of generating a vector of sentences,
                # which can be interpreted as doing a convolution operation,
                # but not adding up the results, the convolution kernel is input_mask.
